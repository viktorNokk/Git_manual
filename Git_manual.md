...
+ **-m** "message" - позволяет сразу передать commit message. 
...
+ **git diff** commitA commitB - показывает разницу между коммитами А и B
...
+ **git remote set-url origin** https://github.com/microsindotnet/gitnew - В этом случае мы попытались создать новый remote с именем "origin", когда remote с таким именем уже существует. Чтобы исправить эту ошибку, мы должны удалить существующий remote, который называется "origin", и добавить новый, либо должны поменять URL существующего remote
...
### Примеры: 
`git ceckout main^` - перейти на один коммит выше

`Git checkout HEAD~4` - на 4 коммита выше
...
5. Производим все изменения только в этой отдельной ветке
